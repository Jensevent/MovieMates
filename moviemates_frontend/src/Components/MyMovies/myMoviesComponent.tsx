...
class myMovieComponent extends Component {
    ...
    LoadButton(movieID : Number, watched? : boolean  ){
        if(watched === true){
            return(
                <Button disabled variant="success" id="WatchedMovie">Watched</Button>
            )
        }
        else{
            return(
                <ToggleButton type="radio" id="WatchMovie" value={movieID.toString()} onChange={(e) => this.WatchMovie(Number(e.target.value))}>Watched?</ToggleButton>
            )
        }
    }
